...
# Plot sample
sample.plot(folder='results/sample_plot', colorParam='centralRing1_Shift', sortByColors=True, plotIndividualParams={}, maxIndivPlotCount=10)
...
sampling.calcForAllxyzInFolder(xyzfolder='xyz', spectralProgram='fdmnes', continueCalculation=True, calcSampleInParallel=2, generateOnly=False, calcFolder='results/xyz_calc', outputFolder='results/xyz_calc_result', **p)
...
smoothParams = {'Gamma_hole': 4.23, 'Ecent': 53, 'Elarg': 24, 'Gamma_max': 24, 'Efermi': 7109, 'shift': 152}
smoothed_sample.spectra = smoothLib.smoothDataFrame(smoothParams, sample.spectra, smoothType='fdmnes', expSpectrum=exp_spectrum, fitNormInterval=[7100,7200], norm=0.031)

# Smooth of experimental spectrum keeping the peak height
s = readSpectrum(f'data/LC42.nor', energyColumn=0, intensityColumn=3)
ss = smoothLib.smoothExp(s, noiseSampleIntervals=[4990,1,[4900,4965], np.inf,5,[5040,5200]], noiseConfidenceLevel=0.9999, debugFileName=f'results/exp_smooth_debug.png')
...
result = curveFitting.substractBaseAuto(s.x, s.y, usePositiveConstrains=True, plotFileName=f'results/preedge_auto/LC39.png', sepFolders=False, debug=False)

# Pre-edge extraction (with turning)
s = readSpectrum(f'data/LC39.nor', energyColumn=0, intensityColumn=3)
result = curveFitting.substractBase(s.x, s.y, peakInterval=[4965,4974], baseFitInterval=[4955,4980], model='arctan', usePositiveConstrains=True, plotFileName=f'results/preedge_manual/LC39.png')

# +
# TODO: adf smooth
# -

# Larch Mback normalization
spectrum = readSpectrum('data/TiCl4THF2.txt')
e = spectrum.x
flat_spectrum = curveFitting.mback(spectrum, pre=[4902, 4952], post=[5000, e[-1]], e0=4983, element='Ti', edge='K', deg=3)
...
flat_spectrum, chik, extra = curveFitting.autobk(spectrum, rbkg=1, kmin=0, kmax=None, kweight=2)
k = chik.x
...
descriptor.addDescriptors(sample, [
...
descriptor.plotDescriptors2d(known.params, descriptorNames=['pca1', 'efermi_e'], labelNames=[label], textColumn='Compound', unknown=unknown.params, folder_prefix='results/descriptors')

# Pre-edge spectrum and descriptors
sample = readSample('data/sampleDescriptors')
descriptor.calcPreedge(sample, plotFolder='results/pre-edge_descriptors', debug=False, inplace=True)
label = 'Activity'
sample.saveToFolder('results/sample_with_pre-edge', plot=True, colorParam=label)
print(sample.paramNames)
known, unknown = sample.splitUnknown(columnNames=[label])
descriptor.plotDescriptors2d(known.params, descriptorNames=['pe area', 'pe center'], labelNames=[label], textColumn='Compound', unknown=unknown.params, folder_prefix='results/pre-edge descriptors')



# !/opt/anaconda/bin/jupytext --to py list.ipynb
